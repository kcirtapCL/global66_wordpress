<?php
$social_network = new WP_Query(
	array(
		'post_type' => 'redes_sociales',
		'meta_key'  => 'order',
		'order'     => 'ASC',
		'orderby'   => 'meta_value'
	)
);
?>
<footer>
    <div class="bg-neutral-1 py-12">
        <div class="container mx-auto">
            <div class="flex flex-col items-center lg:flex-row lg:justify-between">
                <div class="flex flex-col items-center lg:flex-row">
                    <svg width="50" height="50" viewBox="0 0 50 50" fill="none" xmlns="http://www.w3.org/2000/svg"
                         style="width: 50px;height: 50px">
                        <circle cx="25" cy="25" r="25" fill="white"/>
                        <path d="M42.4276 24.5374C42.1402 21.2136 39.3284 18.6035 35.8976 18.6035C33.687 18.6035 31.7329 19.6896 30.548 21.3493C29.8495 22.3303 29.4118 23.5039 29.3499 24.787C29.3455 24.8878 29.3411 24.9929 29.3411 25.0936C29.3411 27.3664 27.4842 29.2057 25.1941 29.2057C25.1897 29.2057 25.1897 29.2057 25.1897 29.2057H25.1852C22.8995 29.2057 21.0383 27.3664 21.0383 25.0936C21.0383 24.9929 21.0338 24.8878 21.0294 24.787C20.9719 23.5039 20.5342 22.3303 19.8313 21.3493C18.6597 19.6896 16.7056 18.6035 14.495 18.6035C11.0642 18.6035 8.25242 21.2136 7.96505 24.5374C7.96505 24.5374 7.94294 24.8045 7.94736 24.8702C7.99157 25.4877 8.50442 25.9826 9.14106 25.9826C9.80865 25.9826 10.3215 25.4395 10.3436 24.7914C10.4232 23.7272 10.9139 22.7769 11.6567 22.0938C12.4038 21.4106 13.3986 20.9902 14.4906 20.9902C16.6746 20.9902 18.4652 22.6675 18.6243 24.7914L18.642 25.1242C18.6464 26.5125 19.0885 27.7912 19.8401 28.8422C21.025 30.502 22.9835 31.588 25.1897 31.588C25.1941 31.588 25.1941 31.588 25.1941 31.588H25.1985C27.4046 31.588 29.3632 30.502 30.548 28.8422C31.2996 27.7912 31.7417 26.5125 31.7462 25.1242L31.7639 24.7914C31.923 22.6631 33.7136 20.9902 35.8976 20.9902C36.9896 20.9902 37.9888 21.4106 38.7315 22.0938C39.4743 22.7769 39.965 23.7272 40.0446 24.7914C40.0711 25.4395 40.584 25.9826 41.2471 25.9826C41.8838 25.9826 42.3966 25.4921 42.4408 24.8702C42.4497 24.8045 42.4276 24.5374 42.4276 24.5374Z"
                              fill="#475694" stroke="#475694" stroke-width="0.5"/>
                    </svg>
                    <p class="text-3xl lg:text-4xl font-alternate font-bold text-neutral-12 leading-none my-8 text-center lg:m-0 lg:ml-6"><?php echo get_configurations( "slogan" ) ?></p>
                </div>
                <a href="https://transferencias.global66.com/signIn"
                   class="block bg-accent-2 rounded-md font-semibold text-neutral-12 text-lg px-12 py-4 transition-all duration-300 ease-in-out hover:bg-accent-4"
                   target="_self">Enviar dinero</a>
            </div>
            <div class="flex flex-col items-center text-center pt-16 lg:items-start lg:text-left lg:flex-row lg:pt-24">
                <div class="flex-auto mb-8 lg:mb-0">
                    <p class="font-semibold text-sm text-neutral-12 text-opacity-50 uppercase mb-2 lg:mb-5">Features</p>
                    <ul class="list-none space-y-2">
                        <li>
                            <a href="https://global66.com/referidos"
                               class="font-normal text-16 text-neutral-12 hover:text-gray-6 transition-all duration-300 ease-in-out"
                               target="_self">Invita y gana</a>
                        </li>
                        <li>
                            <a href="https://global66.com/embajadores"
                               class="font-normal text-16 text-neutral-12 hover:text-gray-6 transition-all duration-300 ease-in-out"
                               target="_self">Embajadores</a>
                        </li>
                        <li>
                            <a href="https://global66.com/salondelafama"
                               class="font-normal text-16 text-neutral-12 hover:text-gray-6 transition-all duration-300 ease-in-out"
                               target="_self">Club de fundadores</a>
                        </li>
                        <li>
                            <a href="https://global66.com/que-nos-respalda"
                               class="font-normal text-16 text-neutral-12 hover:text-gray-6 transition-all duration-300 ease-in-out"
                               target="_self">¿Qué nos respalda?</a>
                        </li>
                        <li>
                            <a href="https://global66.com/glosario"
                               class="font-normal text-16 text-neutral-12 hover:text-gray-6 transition-all duration-300 ease-in-out"
                               target="_self">Glosario</a>
                        </li>
                    </ul>
                </div>
                <div class="flex-auto mb-8 lg:mb-0">
                    <p class="font-semibold text-14 text-neutral-12 text-opacity-50 uppercase mb-2 lg:mb-5">Empresa</p>
                    <ul class="list-none space-y-2">
                        <li>
                            <a href="https://global66.com/nuestro-equipo"
                               class="font-normal text-16 text-neutral-12 hover:text-gray-6 transition-all duration-300 ease-in-out"
                               target="_self">Nuestro equipo</a>
                        </li>
                        <li>
                            <a href="https://global66.teamtailor.com/" rel="nofollow noreferrer"
                               target="_blank"
                               class="font-normal text-16 text-neutral-12 hover:text-gray-6 transition-all duration-300 ease-in-out">Trabaja
                                con nosotros</a>
                        </li>
                        <li>
                            <a href="https://global66.com/compliance"
                               class="font-normal text-16 text-neutral-12 hover:text-gray-6 transition-all duration-300 ease-in-out"
                               target="_self">Compliance 20393</a>
                        </li>
                    </ul>
                </div>
                <div class="flex-auto mb-8 lg:mb-0">
                    <p class="font-semibold text-14 text-neutral-12 text-opacity-50 uppercase mb-2 lg:mb-5">Ayuda</p>
                    <ul class="list-none space-y-2">
                        <li>
                            <a href="https://global66.com/blog"
                               class="font-normal text-16 text-neutral-12 hover:text-gray-6 transition-all duration-300 ease-in-out"
                               target="_self">Blog</a>
                        </li>
                        <li>
                            <a href="https://global66.com/faqs"
                               class="font-normal text-16 text-neutral-12 hover:text-gray-6 transition-all duration-300 ease-in-out"
                               target="_self">Preguntas frecuentes</a>
                        </li>
                        <li>
                            <a href="https://global66.com/centro-ayuda"
                               class="font-normal text-16 text-neutral-12 hover:text-gray-6 transition-all duration-300 ease-in-out"
                               target="_self">Centro de ayuda</a>
                        </li>
                        <li>
                            <a href="https://global66.com/tipo-cambio"
                               class="font-normal text-16 text-neutral-12 hover:text-gray-6 transition-all duration-300 ease-in-out"
                               target="_self">Tipo de cambio</a>
                        </li>
                        <li>
                            <a href="https://global66.com/swift"
                               class="font-normal text-16 text-neutral-12 hover:text-gray-6 transition-all duration-300 ease-in-out"
                               target="_self">Swift</a>
                        </li>
                    </ul>
                </div>
                <div class="flex-auto">
                    <p class="font-semibold text-14 text-neutral-12 text-opacity-50 uppercase mb-3 lg:mb-5">Remesas</p>
                    <ul class="list-none space-y-2">
                        <li>
                            <a href="https://global66.com/transferencias-internacionales"
                               class="font-normal text-16 text-neutral-12 hover:text-gray-6 transition-all duration-300 ease-in-out"
                               target="_self">Transferencias internacionales</a>
                        </li>
                        <li>
                            <a href="https://global66.com/enviar-dinero"
                               class="font-normal text-16 text-neutral-12 hover:text-gray-6 transition-all duration-300 ease-in-out"
                               target="_self">Enviar dinero al extranjero</a>
                        </li>
                        <li>
                            <a href="https://global66.com/remesas"
                               class="font-normal text-16 text-neutral-12 hover:text-gray-6 transition-all duration-300 ease-in-out"
                               target="_self">Remesas internacionales</a>
                        </li>
                        <li>
                            <a href="https://global66.com/giros-internacionales"
                               class="font-normal text-16 text-neutral-12 hover:text-gray-6 transition-all duration-300 ease-in-out"
                               target="_self">Giros internacionales</a>
                        </li>
                    </ul>
                </div>
            </div>
            <div class="flex flex-row items-center justify-between pt-8">
                <div>
                    <p class="font-semibold text-14 text-neutral-12 text-opacity-50 uppercase mb-3 lg:mb-5">Follow us</p>
                    <ul class="list-none mb-6 space-x-1">
		                <?php if ( $social_network->have_posts() ): while ( $social_network->have_posts() ): $social_network->the_post(); ?>
                            <li class="inline-flex bg-neutral-12 hover:bg-gray-8 rounded-lg hover:rounded-none transition-all duration-300 ease-in-out">
                                <a href="<?php echo get_field( "website" ) ?>" target="_blank" rel="nofollow noreferrer"
                                   class="w-10 h-10 text-2xl flex items-center justify-center text-neutral-2"><?php echo get_field( "icon" ) ?></a>
                            </li>
		                <?php endwhile; endif; wp_reset_query(); ?>
                    </ul>
                </div>
                <div>
                    <ul class="list-none">
                        <li class="inline-flex">
                            <a href="https://www.fintechile.org/" target="_self" rel="nofollow noreferrer"
                               class="fintechile-icon block w-40 h-12 bg-no-repeat bg-center bg-auto"
                               style="text-indent:-9999px;background-image: url('<?php echo get_template_directory_uri() ?>/images/members/fintechile.svg')">FinteChile</a>
                        </li>
                        <li class="inline-flex">
                            <a href="https://www.ccs.cl/" target="_self" rel="nofollow noreferrer"
                               class="ccs-icon block w-40 h-12 bg-no-repeat bg-center bg-contain"
                               style="text-indent:-9999px;background-image: url('<?php echo get_template_directory_uri() ?>/images/members/ccs.svg')">Camara
                                de Comercio de Santiago</a>
                        </li>
                    </ul>
                </div>
            </div>
        </div>
    </div>
    <div class="bg-neutral-2 py-6">
        <div class="container mx-auto">
            <div class="text-center text-sm text-neutral-12 font-medium">
                <p>Copyright © <span class="year"></span> Global66.</p>
                <p>Av. Apoquindo 4615, Of. 701 - Piso 7, Las Condes, Santiago.</p>
                <div class="inline-flex flex-row text-xs space-x-2">
                    <a href="https://global-files-statics.s3.amazonaws.com/T%C3%A9rminos+y+Condiciones.pdf"
                       target="_blank">Términos y condiciones</a>
                    <span>-</span>
                    <a href="https://global66.com/privacidad" target="_blank">Políticas de privacidad</a>
                </div>
            </div>
        </div>
    </div>
</footer>

<?php wp_footer() ?>
</body>
</html>
